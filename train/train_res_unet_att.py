...
# Import the model and dataset functions
from models.resunet_att_model import get_resnet_unet
...
def setup_logging():
    ...
    log_file = f"logs/training_resnet_att_{timestamp}.log"
...
def setup_batch_logging(timestamp):
    """Set up CSV logging for batch-level metrics"""
    batch_log_file = f"logs/batch_metrics_resnet_att_{timestamp}.csv"
...
def save_prediction_sample(images, masks, preds, sample_idx, class_names, phase=0, epoch=0):
    ...
    os.makedirs('results_resnet_att/predictions', exist_ok=True)
    
    # Save figure
    plt.tight_layout()
    plt.savefig(f'results_resnet_att/predictions/phase{phase}_epoch{epoch}_sample{sample_idx}.png')
    plt.close()
...
def train():
    ...
    os.makedirs('results_resnet_att', exist_ok=True)
    
    # Create data loaders with simple data loader that gets aleady augmented data from the augmented dataset directory | USE create_augmented_data_loaders IF U DO NOT HAVE THE AUGMENTED DATASET DIRECTORY
    ...
    for epoch in range(phase1_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'results_resnet_att/best_phase1_resnet101_model.pth')
    ...
    for epoch in range(phase2_epochs):
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), 'results_resnet_att/best_resnet101_model.pth')
            logging.info("  New best model saved!")
    
    # Save final model
    torch.save(model.state_dict(), 'results_resnet_att/final_resnet101_model.pth')
    ...
    plt.legend()
    plt.savefig('results_resnet_att/loss_curve.png')
    logging.info("Loss curve saved to results_resnet_att/loss_curve.png")
...
def test_model(model, test_loader, device, class_names):
    ...
    # Save results to CSV
    with open('results_resnet_att/test_results.csv', 'w', newline='') as f:
        writer = csv.writer(f)
