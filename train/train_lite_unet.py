...
def setup_logging():
    ...
    log_file = f"logs/training_liteUnet_{timestamp}.log"
...
def setup_batch_logging(timestamp):
    """Set up CSV logging for batch-level metrics"""
    batch_log_file = f"logs/batch_metrics_liteUnet_{timestamp}.csv"
...
def train():
    ...
    train_loader, val_loader, test_loader = create_simple_data_loaders(batch_size=8, num_workers=2)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'results_liteUnet/best_segmentation_model.pth')
            logging.info("  New best model saved!")
    
    # Save final model
    torch.save(model.state_dict(), 'results_liteUnet/final_segmentation_model.pth')
    ...
    plt.legend()
    plt.savefig('results_liteUnet/loss_curve.png')
    logging.info("Loss curve saved to results_liteUnet/loss_curve.png")
...
def save_prediction_sample(images, masks, preds, sample_idx, class_names):
    ...
    plt.tight_layout()
    plt.savefig(f'results_liteUnet/sample_{sample_idx}.png')
    plt.close()
